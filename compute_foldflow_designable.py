...
def process(results_dir):
    ...
    with open(designable_text_path, "w") as file:
        for pmpn_csv_path in tqdm(results_dir.glob("length_*/sample_*/self_consistency/sc_results.csv")):
            length = str(pmpn_csv_path.parents[2].stem.split("_")[1])
            sample_id = str(pmpn_csv_path.parents[1].stem.split("_")[1])
            pmpnn_df = pd.read_csv(pmpn_csv_path).sort_values('rmsd', ascending=True)
            best_row = pmpnn_df.iloc[0]
            total += 1
            if best_row.rmsd < 2.0:
                designable += 1
                sample_path = results_dir.parents[1] / best_row.sample_path
                file.write(f"{sample_path}\n")
                shutil.copy(sample_path, pmpnn_designable_dir / f"len_{length}_id_{sample_id}.pdb")
...
def run():
    base_dir = Path("/data/rbg/users/jyim/projects/FoldFlow/results/")
