...
from multiflow.data import so3_jumps
...
class Interpolant:
    ...
    def __init__(self, cfg):
        ...
        self._vmf_path = so3_jumps.VMFPath(**cfg.vmf)
        self._placement_grid = so3_jumps.get_uniform_quaternion_bins(
            r=1, N=self._rots_cfg.num_jump_bins
        )[0].float()
        self._vmf_path.compute_cashed_dt_pt(self._placement_grid)
    ...
    def set_device(self, device):
        self._device = device
        if str(self._placement_grid.device) != device:
            self._placement_grid = self._placement_grid.to(device)
    ...
    def _rots_step(self, d_t, t, rotmats_1, rotmats_t):
        num_res = rotmats_1.shape[1]
        quats_t_1 = rot_to_quat(rotmats_t)
        # Rot jump sampling
        jump_weight = self._rots_cfg.jump_weight
        if jump_weight > 0.0:
            jump_step = d_t * jump_weight
            pred_quats_1 = rot_to_quat(rotmats_1)
            quats_t = rot_to_quat(rotmats_t)
            flat_time_vec = t.repeat(1, num_res).reshape(-1)
            flat_quats_1 = pred_quats_1.reshape(-1, 4)
            flat_quats_t = quats_t.reshape(-1, 4)
            with torch.enable_grad():
                jump_intensity = self._vmf_path.lambda_t(flat_quats_1, flat_quats_t, flat_time_vec)
                probs = self._vmf_path.cached_J_t(flat_quats_1, self._placement_grid, flat_time_vec, self._rots_cfg.jump_temp)
            unif_samples = torch.rand_like(jump_intensity)
            jump_mask = (unif_samples <= torch.clip(jump_step * jump_intensity, min=0.0, max=1.0))
            cum_probs = probs.cumsum(dim=-1)
            cum_probs = torch.concat([torch.zeros_like(cum_probs[...,0]).unsqueeze(-1),cum_probs], dim=-1)
            random_samples = torch.rand(size=cum_probs.shape[:-1],device=self._device)
            smaller_mask = (cum_probs <= random_samples[..., None])
            bin_mask = ((smaller_mask[...,:-1] & (~smaller_mask[...,1:])))
            jump_samples = (bin_mask[..., None] * self._placement_grid[None, None]).sum(dim=-2)
            quats_t_2 = (jump_mask[..., None] * jump_samples) + ((~jump_mask)[..., None] * quats_t_1)
            jump_rotmats_t_2 = quat_to_rot(quats_t_2)
        else:
            jump_mask = torch.zeros(num_res, device=self._device).bool()
            jump_rotmats_t_2 = torch.zeros(num_res, 3, 3, device=self._device)

        flow_weight = self._rots_cfg.flow_weight
        if flow_weight > 0.0:
            flow_step = d_t * flow_weight
            flow_rotmats_t_2 = self._rots_euler_step(
                flow_step, t, rotmats_1, rotmats_t)
        else:
            flow_rotmats_t_2 = rotmats_t
        jump_mask = jump_mask[None, :, None, None]
        rotmats_t_2 = jump_mask * jump_rotmats_t_2 + (~jump_mask) * flow_rotmats_t_2
        return rotmats_t_2
    ...
    def sample(
        ...
        for t_2 in ts[1:]:
            ...
            rotmats_t_2 = self._rots_step(
                d_t, t_1.to(self._device), pred_rotmats_1, rotmats_t_1)
